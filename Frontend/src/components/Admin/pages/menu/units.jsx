...
export default function Units() {
  ...
  const handleToggleModal = () => {
    ...
    if (!showModal) {
      setunitData({ name: "", status: "Active" });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      // toast.error(
      //   error.response?.data?.message || "Error deleting unit."
      // );
      toast.error(t('unitAssigned'));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedUnit) {
        ...
        response = await axios.post(`${API_URL}/api/unit`, {
          AdminId,
          ...unitData,
        });
      ...
      // toast.error(
      //   error.response?.data?.message || "Error saving unit."
      // );
      toast.error(t('unitExists'));
    }
  };
  ...
  return (
    <div className="flex flex-col h-full p-3 bg-gray-950">
      ...
      {/* Table Container fills remaining height with scroll */}
          ...
          <div className="relative p-8 bg-gray-900 border border-gray-800 rounded-lg shadow-md max-w-md w-full">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-xl font-semibold text-white">
                {selectedUnit ? t('edit_unit') : t('add_unit')}
              </h1>
              <RxCross2
                size={25}
